...
Once again, the time to do this is while everything is long, so that you only have one column to work with.^[`abbreviate` comes from base R; it returns an abbreviated version of its (text) input, with a minimum length of its second input. If two abbreviations come out the same, it makes them longer until they are different. It has some heuristics to get things short enough: remove vowels, then remove letters on the end.] Again, you can do this at the end, but it's more work, because you are now renaming columns:
