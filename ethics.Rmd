...
```{r}
predictions(decide.1, variables = "gender")
```
...
predictions(decide.4, variables = c("gender", "scenario")) %>% 
  select(predicted, gender, scenario)
```
...
```{r}
predictions(decide.3, variables = c("idealism", "relativism"))
```

except that there are 5 representative values for each variable, and all the combinations of those, so 25 rows, a lot to get your head around. We can supply two custom values for each variable, and then we'll only get four combinations, for example like this:
...
```{r}
new <- datagrid(newdata = decide, idealism = c(5.6, 7.5), relativism = c(5.4, 6.8))
new
```

The most common categories `Female` and `theory` were used for the other variables:

```{r}
predictions(decide.3, newdata = new) %>% 
  select(predicted, relativism, idealism)
```
