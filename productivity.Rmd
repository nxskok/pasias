...
(a)[2] Read in and display at least some of the data.
...
(b)[3] Plot productivity improvement for this year against last
...
(c)[3]<a name="part:graph-comment">*</a> 
...
(d)[3] Fit an analysis of covariance model without interaction. Run
...
(e)[3] Now look at the `summary` of your analysis of
...
* The two estimates for `expenditure` `low` and
`moderate` are both positive, so when expenditure on R\&D is
low or moderate, the productivity increase will be higher this year
(for the same productivity increase last year). Going back to the
graph, if you extend the lines for `low` and `high` to
...
baseline level `high`, rather than comparing all three
...
(f)[3] Add an interaction between `last` and
...
from the interaction. Once again `high` is the baseline. These
say how the *slopes* for `last` for the other groups
differ from the slopes for `last` for `high`
...
expenditure and high expenditure. This says that the line for low
expenditure is this much less steep than the line for high
...
in slopes, $-0.40$ and $-0.09$, are the smallest things in size out of
...
(oh, how I wish I'd given those variables shorter names), and then:
