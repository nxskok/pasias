...
The lists of values are not very long, so I think it's fine to just have one `datagrid` line with the values inside. If you want to save them first, go ahead:

```{r}
angers <-  c("y", "n")
anxieties <- c(40, 50, 60) 
new <- datagrid(model = ha.1, anger = angers, anxiety = anxieties)
new
```

My strategy is to give the vectors of values *plural* names. If you want, you can use the same names as the variables in the model, but then your `datagrid` has stuff like `anger = anger` in it, which you might find confusing.

Then, the predictions:

```{r}
predictions(ha.1, newdata = new)
```

Extra: In the help file for `predictions`, you will see it done in one step, with code like

```
predictions(ha.1, newdata = datagrid(anger = angers, ...))
```
