...
posters, paintings and other paraphernalia.\endnote{This is a fancy
word for "stuff".} A memory test was given to all subjects
...
`%*%` means "matrix multiply".\endnote{In R, the percents
around something mean "special version of", thus this notation for
"matrix multiply" and the pipe symbol. A regular * when used for
multiplying matrices in R will multiply them element by element.} Thus,
...
model, so `predict` works the same here as in regression.}
...
by saying `se=F` inside the `geom_smooth`.} They are
