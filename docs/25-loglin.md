...
`class`. This is actually OK, because we can say ``if a voter
was of lower middle class'' and then talk about the relationship
...
why there was a `vote:age:class` interaction: `` the way
someone votes depends on the *combination* of age and social
class''. 
...
for their party, and would never think of voting for the ``other
side'', in the same way that many Americans vote either Democrat or
