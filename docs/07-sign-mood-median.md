...
```r
tibble(meds = seq(55, 58, 0.25)) %>% mutate(pvals = map_dbl(meds, pval_sign, times, 
    time))
```
...
```r
bags %>% mutate(less = (chips < 1100)) %>% group_by(less) %>% summarize(howmany = n())
```
...
```
##  [1] 43.55162 16.51427 73.87807 41.06865 56.69978 57.15163 47.14486 41.88923
##  [9] 40.08952 49.88583
```
...
```r
tibble(x) %>% count(x < 40) %>% summarize(the_min = min(n)) %>% mutate(is_rejected = (the_min <= 
    1))
```
...
```r
set.seed(457299)
rerun(1000, rnorm(10, 50, 15)) %>% map(~tibble(x = .)) %>% map(~count(., x < 40)) %>% 
    map(~summarize(., the_min = min(n))) %>% map(~mutate(., is_rejected = (the_min <= 
    1 | the_min == 10))) %>% map_lgl(~pull(., is_rejected)) %>% tibble(was_true = .) %>% 
    count(was_true)
```
...
is_reject = function(x) {
    tibble(x = x) %>% count(x < 40) %>% summarize(the_min = min(n)) %>% mutate(is_rejected = the_min <= 
        1 | the_min == 10) %>% pull(is_rejected)
}
```
...
```r
set.seed(457299)
rerun(1000, rnorm(10, 50, 15)) %>% map_lgl(~is_reject(.)) %>% tibble(rejected = .) %>% 
    count(rejected)
```
...
```r
tibble(rl) %>% ggplot(aes(sample = rl)) + stat_qq() + stat_qq_line()
```


\includegraphics{07-sign-mood-median_files/figure-latex/unnamed-chunk-50-1} 
...
```r
pvals <- rerun(1000, rdoublex(50, mu = 0.5)) %>% map(~t.test(., mu = 0)) %>% map_dbl("p.value")
```
...
```r
pvals_sign <- rerun(1000, rdoublex(50, mu = 0.5)) %>% map_dbl(~pval_sign0(0, .))
```
...
\includegraphics{07-sign-mood-median_files/figure-latex/unnamed-chunk-60-1} 
...
```r
ggplot(math, aes(sample = phobia)) + stat_qq() + stat_qq_line() + facet_wrap(~course, 
    ncol = 1, scales = "free")
```


\includegraphics{07-sign-mood-median_files/figure-latex/unnamed-chunk-70-1} 
...
\includegraphics{07-sign-mood-median_files/figure-latex/unnamed-chunk-75-1} 
...
```r
ggplot(instr, aes(sample = score)) + stat_qq() + stat_qq_line() + facet_wrap(~group, 
    ncol = 1)
```


\includegraphics{07-sign-mood-median_files/figure-latex/unnamed-chunk-76-1} 
