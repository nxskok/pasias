...
low to high or high to low (it doesn't matter).\endnote{The
results might look different which way you go, but it won't
make any *material* difference.} I'm going negative to
...
other requirements I put in the question.\endnote{If you don't take
out the `NA` values, they are shown separately on the end of
the `summary` for that column.}
...
difference.\endnote{There are three political parties; using the first
as a baseline, there are therefore $3-1=2$ coefficients for each variable.}
...
Conservatives are nicknamed Tories\endnote{It amuses me that
Toronto's current mayor is politically a Tory.}). The Whigs became Liberals. At
...
to specify a frequency variable.\endnote{Discovered by me two minutes
ago.}
...
smallish, as befits an effect that is not significant.\endnote{There were only
216 alligators total, which is a small sample size for this kind of
thing, especially with all those parameters to estimate.}
...
Asian,\endnote{I'm always amused at how Americans put all Asians
into one group.}  Hispanic, White).
