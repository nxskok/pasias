...
What about if we plot the median *and* quartiles for each visibility value? I don't know how to do that yet,  but I know how to *calculate* them, so let's do that first: `r tufte::margin_note("The \emph{factor} thing is not needed here, because grouping will use each of the distinct values even if the variable is really quantitative.")`
...
```
This has a row for each summary, but unfortunately there is no indication of which is which, so we need to put them in ourselves:`r tufte::margin_note("Or be cleverer than I am capable of.")`
...
Extra 3: an attempt to be clever.`r tufte::margin_note("And thus most likely doomed to failure.")`
