...
`r tufte::margin_note("As *str_c* or *paste* do, actually, but the advantage of *unite* is that it gets rid of the other columns, which you probably no longer need.")`
```{r }
crickets %>% 
...
`r tufte::margin_note("You could      just as well make the point that the text 20.8 contains the      number 20.8 and nothing else, so that `parse_number`      will pull out 20.8 as a number. If that logic works for you, go      with it.")`
...
`r tufte::margin_note("Mixes  up.} two things: the textsl{exclamationis")` crickets do have a
