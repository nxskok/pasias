...
The $F$-test is significant, meaning that there are some differences among ` r tufte::margin_note("There might be differences between two things, but among three or more.")` the means, and Tukey's method will enable us to see which ones differ:
...
In this one, the three things in the column `data` are each *dataframes*, `r tufte::margin_note("Like those Russian dolls.")` containing the column called `Age` from the original dataframe. These are the ages at death of the writers of that particular `Type`. These are the things we want bootstrap samples of.
