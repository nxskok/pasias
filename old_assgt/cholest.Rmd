\question The data set at
[http://ritsokiguess.site/datafiles/STAC32/cholest.csv](http://ritsokiguess.site/datafiles/STAC32/cholest.csv) contains cholesterol
...
```{r}
my_url <- "http://ritsokiguess.site/datafiles/STAC32/cholest.csv"
...
This column also has missing values (at the bottom), so here I've asked to remove the missing values\endnote{In R, missing values are labelled NA, and ``rm'' is Unix/C shorthand for ``remove''.} before working out the mean. Otherwise the mean is, unhelpfully, missing as well.
...
```
  ...
  \p Make a suitable plot of the cholesterol levels of the
...
distribution is somewhat skewed to the right.
  ...
  \p It is recommended that people in good health, such as the
...
Writing out all the steps correctly shows that you understand the process. Anything less doesn't. 
  ...
  \p What values could the population mean cholesterol level take? You
...
\href{https://www2.southeastern.edu/Academics/Faculty/dgurney/Math241/StatTopics/SciNot.htm}{this},
in particular the piece at the bottom.
  \eS
  
  \p Explain briefly why you would be reasonably happy to trust
  ...
  Extra 2: By now you are probably thinking that this is very
    ...
    don't know what the population looks like.\endnote{If you did, all
    your problems would be over.} But we can fake it up, in a couple
  of ways: (a) we can play what-if and pretend we know what the population
  looks like (to get some understanding for "populations like
  that"), or (b) we can use a technique called the "bootstrap" that
  ...
  To illustrate my (a), let's pretend the population looks like this,
  with a flat top:
...
size \emph{three}, not in any sense a large sample, from this
...
```{r}
rerun(1000, sample(cholest$control, 30, replace = TRUE)) %>% 
  map_dbl(~mean(.)) %>% 
  enframe() %>% 
  ggplot(aes(x=value)) + geom_histogram(bins=12)
``` 
