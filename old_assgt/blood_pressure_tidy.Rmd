...
The data as I originally received it is in \url{http://ritsokiguess.site/datafiles/STAC32/blood_pressure2.csv}.

\bP

\p Read in and display the data as originally received.
...
```{r}
my_url <- "http://ritsokiguess.site/datafiles/STAC32/blood_pressure2.csv"
bp0 <- read_csv(my_url)
bp0
```
...
\p Describe briefly how the data you read in is not tidy, bearing in mind how the data were collected and how they would be analysed.
...
\p Produce a tidy dataframe from the one you read in from the file. (How many rows should you have?)
...
\p What kind of test might you run on these data? Explain briefly.
...
\p Draw a suitable graph of these data.
...
```
Most of the lines are going uphill, so most of the `after` measurements are less than the corresponding `before` ones.\endnote{This is known in the trade as a "spaghetti plot" because the lines resemble strands of spaghetti.}
