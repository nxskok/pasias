\question Some people believe that poets, especially female poets, die younger than other types of writer. [William Butler Yeats](https://en.wikipedia.org/wiki/W._B._Yeats)\endnote{An Irish, that is to say, Gaelic, poet (see below), but a male one.} wrote:

> She is the Gaelic\endnote{Gaelic is a language of Scotland and Ireland, and the culture of the people who speak it.} muse, for she gives inspiration to those she persecutes. The Gaelic poets die young, for she is restless, and will not let them remain long on earth.
...
The data are in [http://ritsokiguess.site/datafiles/STAC32/writers.csv](http://ritsokiguess.site/datafiles/STAC32/writers.csv).
...
```{r}
my_url <- "http://ritsokiguess.site/datafiles/STAC32/writers.csv"
...
\p Make a suitable plot of the ages and types of writing.
...
\p Obtain a summary table showing, for each type of writing, the number of writers of that type, along with the mean, median and standard deviation of their ages at death.
...
\p Run a complete analysis, starting with an ordinary (not Welch) analysis of variance, that ends with a conclusion in the context of the data and an assessment of assumptions.
...
The $F$-test is significant, meaning that there are some differences among\endnote{There might be differences *between* two things, but *among* three or more.} the means, and Tukey's method will enable us to see which ones differ:
...
In this one, the three things in the column `data` are each *dataframes*,\endnote{Like those Russian dolls.} containing the column called `Age` from the original dataframe. These are the ages at death of the writers of that particular `Type`. These are the things we want bootstrap samples of.
...
and then the last stage is to do facetted normal quantile plots of these, facetted by `Type`:
