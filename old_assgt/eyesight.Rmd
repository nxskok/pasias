\question Do people see on average better with their left eye or their right eye, or is there no difference? To find out, 15 subjects were shown a sequence of images, some to their left eye and some to their right (with a blindfold on the other eye). The subjects were asked to identify some objects in each image, and were given an overall score for each eye, based on their ability to identify objects with each eye. (A higher score is better.) Data in \url{http://ritsokiguess.site/datafiles/STAC33/eyesight.csv}.
...
```{r}
my_url <- "http://ritsokiguess.site/datafiles/STAC33/eyesight.csv"
...
\p Explain briefly why looking at differences (say right minus left) makes sense for these data, and calculate and save a dataframe with the differences added to it.
...
\p Make a suitable normal quantile plot, and describe what it tells you.
...
```{r}
rerun(1000, sample(sight$difference, replace = TRUE)) %>% 
  map_dbl(~median(.)) -> medians
```

This gives you a vector of medians that you should probably look at (for yourself), but it's the kind of thing that's rather long to be displaying on something you'd hand in.

Extra: the `rerun` way with `map` is how I taught you to do simulations, but now that I think about it, I think the `rowwise` way is easier to understand. Here's how that goes:
...
The code is longer, but there is not much unfamiliar in there. The steps are:
...
- generate a bootstrap sample for each row. The `sample` part is what we had before, but because we are working in a dataframe and a bootstrap sample is fifteen observations rather than just one, we are making a list-column and thus the `list` has to go on the front
- work out the median of each bootstrap sample. Remember, the `rowwise` applies until you cancel it,\endnote{This is done using {\tt ungroup}, should you ever need to stop working rowwise. This seems like an odd choice of function, since the usual use of {\tt ungroup} is to undo a {\tt group-by}, but what {\tt ungroup} {\em actually} does is to remove any special properties a dataframe has, including both groups and any rowwise behaviour.} and so this will be the median of the bootstrap sample on each row, one at a time. 


As ever, if you want to see what's going on, run this one line at a time. 

I am coming to like this way better than the one with `rerun` because this stays firmly in the dataframe world, rather than producing lists as an intervening step. Also, the next step  is to make a plot, and that is easier if you already have everything in a dataframe.
...
\p Make a histogram of your bootstrap distribution of the median. Use a lot of bins, such as the default 30, for this. What do you notice about the distribution? Why did it come out this way?

\bS

If you don't already have a dataframe, make one first. For this histogram, there is no need to specify a number of bins (unless you want to):
...
The same happens if you do it the `rowwise` way. The plot is a bit easier since the bootstrap distribution of the median is already in a dataframe:
...
and once again bootstrap the median (the `rowwise` way, just because):
...
\p Find a 95% percentile interval for the population median.\endnote{I was also going to have you do a bootstrap-$t$ interval, but I'm not completely convinced I got that right when I was explaining it to you before.}
...
\p Find the BCA 95% confidence interval for the population median difference.
...
$-2$ to 4, in this case like the percentile interval.\endnote{They don't often agree this well, but all of these intervals in this situation but have data values at their endpoints, and all of our data values are integers.} Note how this one is data values also. 

\eS

\p What do your intervals tell us about any possible difference between left eye and right eye in terms of ability to identify objects in images? Do the intervals agree or disagree about this?
