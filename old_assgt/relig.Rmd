\question Many students at a certain university were asked about the importance of religion in their lives (categorized as "not", "fairly", or "very" important), and also about the number of 
hours they spent studying per week. (This was part of a much larger survey.) We want to see whether there is any kind of relationship between these two variables. The data are in [here](http://ritsokiguess.site/datafiles/STAC32/student_relig.csv). 
...
```{r}
my_url <- "http://ritsokiguess.site/datafiles/STAC32/student_relig.csv"
...
```{r}
my_url <- "http://ritsokiguess.site/datafiles/STAC32/student0405.csv"
...
\p[2] Obtain the number of observations and the mean and standard deviation of study hours for each level of importance.
...
\p[2] Comment briefly on how the groups compare in terms of study hours.
...
\p[2] Make a suitable graph of this data set.
...
\p[3] The statistician in this study decided that the data were sufficiently normal in shape given the (very large) sample sizes, but was concerned about unequal spreads among the three groups. 
Given this,
...
\p[2] What do you conclude from your analysis of the previous part, in the context of the data?
...
student %>% nest(-ReligImp) %>% 
  mutate(samp=map(data, ~rerun(1000, sample(.$StudyHrs, replace = T)))) %>% 
  mutate(means=map(samp, ~map_dbl(., ~mean(.x)))) %>% 
  unnest(means) %>% 
  ggplot(aes(sample=means)) + stat_qq() + stat_qq_line() +
...
- generate 1000 bootstrap samples for each group (this is rather hairy, which plays out in the next line, because within each group you have 1000 different bootstrap samples, and we will want the mean of each one in a moment.)
- work out the mean of each bootstrap sample within each group. The inner `map_dbl` works out the mean for each bootstrap sample, and the outer `map` does this for each religious-importance group. Hairy indeed!
- make a column of all the means
