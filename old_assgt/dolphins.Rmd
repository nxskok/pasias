...
```{r}
my_url <- "http://ritsokiguess.site/datafiles/STAC32/dolphins.csv"
...
\p Display only the two columns `mercury` and `sex`.
...
\p Display all the columns whose names have exactly three characters, without naming any columns.
...
\p Display only the mercury levels for the females. 
...
\p What is the mean mercury concentration for all the dolphins whose age is less than 15?
...
\p What is the mean mercury concentration for all  the dolphins whose age is greater than 25?
...
\p Make a suitable graph of these data (all three columns).
...
\p Explain briefly how your graph and your calculations of mean mercury concentration are telling a similar story.
