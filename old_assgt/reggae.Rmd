\question Reggae is a music genre that originated in Jamaica in the late 1960s. One of the most famous reggae bands was Bob Marley and the Wailers. 
...
Each student was also asked to classify their home town as one of "big city", "suburban", "small town", "rural". Does a student's opinion of reggae depend on the kind of home town they come from? The data are in \url{http://ritsokiguess.site/datafiles/STAC32/reggae.csv}.
...
```{r}
my_url <- "http://ritsokiguess.site/datafiles/STAC32/reggae.csv"
...
\p[2] How many students are from each different size of town?
...
\p[2] Make a suitable graph of the two variables in this data frame. 
...
I am taking the point of view that we want to compare ratings within each category of `home`, which I think makes sense here (see discussion below), which breaks my "rule" that the categorical variable with fewer categories should be `x`.\endnote{Perhaps a better word here would be \emph{principle}, to convey the idea that you can do something else if it works better for your purposes.} 
...
\p[2] Discuss briefly why you might prefer to run Mood's median test to compare ratings among home towns.
...
\p[2] Suppose that somebody wanted to run Welch ANOVA on these data. What would be a reasonable argument to support that?
...
\p[2] Run Mood's median test and display the output.
...
\p[3] Explain briefly why running pairwise median tests is a good idea, run them, and display the results.
...
\p Summarize, as concisely as possible, how the home towns differ in terms of their students' ratings of reggae music.
