...
Now you need to think about your reader. I happen to know four of these airports (Chicagoxxx tufte::margin_note{The abbreviation comes from ``O'Hare Field'', which is the original name of Chicago's airport,}
...
```
I did something a little different here: I didn't know what the exact `name` would be, so I asked for the rows where the text `Southwest` was found in the name somewhere.xxx tufte::margin_note{This is actually a regular expression match.}
...
This is rather a lot to think about, but the weather will have to be looked up for each flight. The keyxxx tufte::margin_note{A joke. The most important column, and also the one the database lookup is going to be based on.} column is the one called `time_hour` in `flights`. What does `weather` contain?
...
What about if we plot the median *and* quartiles for each visibility value? I don't know how to do that yet,  but I know how to *calculate* them, so let's do that first:xxx tufte::margin_note{The \emph{factor} thing is not needed here, because grouping will use each of the distinct values even if the variable is really quantitative.}
...
Well, this I can do now.xxx tufte::margin_note{If I remember to put quotes around the names of my new columns, that is.} Let's make this longer, since we want *one* column of delay summaries to plot:
...
```
This has a row for each summary, but unfortunately there is no indication of which is which, so we need to put them in ourselves:xxx tufte::margin_note{Or be cleverer than I am capable of.}
...
Extra 3: an attempt to be clever.xxx tufte::margin_note{And thus most likely doomed to failure.}
