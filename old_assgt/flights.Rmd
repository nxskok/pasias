\question In 2013, there were over 300,000 flights that departed from one of New York City's three airports. The package `nycflights13` contains information about all these flights, as well as details about the airports, airlines and planes referenced in the flights dataset, and also information about the weather in each hour of 2013 (that can be matched up with the flights). Install the package using the usual `install.packages`, and load it. When you load the package, dataframes called `airlines`, `airports`, `flights`, `planes` and `weather` are available to you.

\bP

\p Using the `airports` dataframe, display the names of New York City's three major airports. They have airport codes EWR, JFK, LGA.
...
\p How many flights departed from each of these airports, in the `flights` dataframe? 
...
\p Display the *names* of the five most common destination airports for flights from the New York City airports (taken together), along with the number of flights to each.
...
Now you need to think about your reader. I happen to know four of these airports (Chicago\endnote{The abbreviation comes from ``O'Hare Field'', which is the original name of Chicago's airport,}
...
```
I did something a little different here: I didn't know what the exact `name` would be, so I asked for the rows where the text `Southwest` was found in the name somewhere.\endnote{This is actually a regular expression match.}
...
This is rather a lot to think about, but the weather will have to be looked up for each flight. The key\endnote{A joke. The most important column, and also the one the database lookup is going to be based on.} column is the one called `time_hour` in `flights`. What does `weather` contain?
...
What about if we plot the median *and* quartiles for each visibility value? I don't know how to do that yet,  but I know how to *calculate* them, so let's do that first:\endnote{The \emph{factor} thing is not needed here, because grouping will use each of the distinct values even if the variable is really quantitative.}
...
Well, this I can do now.\endnote{If I remember to put quotes around the names of my new columns, that is.} Let's make this longer, since we want *one* column of delay summaries to plot:
...
```
This has a row for each summary, but unfortunately there is no indication of which is which, so we need to put them in ourselves:\endnote{Or be cleverer than I am capable of.}
...
Extra 3: an attempt to be clever.\endnote{And thus most likely doomed to failure.}
