\question Take your right hand, and stretch the fingers out as far as
...
\url{http://ritsokiguess.site/datafiles/STAC32/handspan.txt}, with handspans
...
```{r}
my_url <- "http://ritsokiguess.site/datafiles/STAC32/handspan.txt"
span <- read_delim(my_url, " ")
span
``` 
  \eS
  
  \p Make a suitable (facetted) normal quantile plot of the data. (Bear in
...
```     
  ...
  \p Discuss briefly whether you might prefer to use Mood's median
...
My take is that the sampling distribution of the sample mean for the females is normal enough, therefore the one for the males is also normal enough, therefore the two-sample $t$-test is actually fine.
  ...
  \p Run Mood's median test. What do you conclude from the test, in the context of the data?
