\question Two groups of students (in a class at a American university) were asked what they thought the population of Canada was. (The correct answer at the time was just over 30 million.)
...
about 18 million. The data are in \url{http://ritsokiguess.site/datafiles/STAC32/anchoring.csv}. The first column contains the country whose population the student was told, and the second contains the student's guess at the population of Canada.
...
```{r}
my_url <- "http://ritsokiguess.site/datafiles/STAC32/anchoring.csv"
...
\p Explain briefly why a Welch $t$-test would be better than a pooled $t$-test in this case.
...
\p Run a suitable Welch $t$-test and display the output.
...
rerun(1000, sample(oz$estimate, replace = TRUE)) %>% 
  map_dbl(~mean(.)) %>% 
  enframe() %>% 
...
Extra 3: looking even further ahead, there is a test that definitely *does* apply here, called Mood's Median Test. You won't have installed the package yet, so this won't work for you just yet ([read ahead](http://ritsokiguess.site/datafiles/STAC33/inference_5_R_slides.pdf) if you want to learn more), but here's how it goes:
