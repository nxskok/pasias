...
this with `anova`, which is next.
 

(f) Pass your fitted model of the last part into
`anova`. This gives you an overall test for each effect. Is
...
Another, perhaps easier, way to do this is with `drop1`:
...
but you could do it by `anova` again, this way:
```{r }
anova(lepro.2)
```
...
```{r }
drugs=levels(factor(lepro$drug))
drugs
pres=c(5,12,20)
...
That was rather an around-the-houses way of getting the names of the
drugs without typing them: turn `drug` from text into a factor,
then pull out its levels. 

Of course, this is probably quicker and certainly easier:
...
I got the possible drugs by using `levels` of the variable
`drug` in the data frame `lepro`, which, with great
discipline, I attached first and detached after. (I might have been
getting the values from `pres` from the data frame too, but
didn't in this case.)
