...
```r
response <- with(weights, cbind(Time0, Time1, 
    Time2, Time3, Time4))
...
##    <dbl> <chr>      <chr> <chr>  <dbl>
##  1     5 thyroxin   Time  3        120
##  2     7 thyroxin   Time  3        138
##  3    10 thiouracil Time  4        133
##  4     5 thyroxin   Time  4        144
##  5    11 thiouracil Time  0         59
##  6     7 thyroxin   Time  0         52
##  7     1 thyroxin   Time  2        121
##  8    14 thiouracil Time  4        108
##  9     6 thyroxin   Time  2         97
## 10     9 thiouracil Time  2        101
## 11     4 thyroxin   Time  2        116
## 12    26 control    Time  0         49
## 13    22 control    Time  0         56
## 14     3 thyroxin   Time  1         75
## 15    21 control    Time  2        100
## 16     9 thiouracil Time  4        122
## 17    12 thiouracil Time  0         51
## 18    27 control    Time  3        139
## 19    22 control    Time  4        151
## 20    26 control    Time  3        112
```
...
##    <dbl> <chr>      <chr>  <dbl> <dbl>
##  1    17 thiouracil Time1     72     1
##  2    26 control    Time3    112     3
##  3     1 thyroxin   Time2    121     2
##  4     8 thiouracil Time2    109     2
##  5    13 thiouracil Time3    100     3
##  6    10 thiouracil Time4    133     4
##  7    25 control    Time4    154     4
##  8     3 thyroxin   Time1     75     1
##  9     1 thyroxin   Time0     59     0
## 10    25 control    Time0     63     0
## 11    13 thiouracil Time2     92     2
## 12    22 control    Time4    151     4
## 13    15 thiouracil Time0     58     0
## 14    20 control    Time2    111     2
## 15    13 thiouracil Time4    119     4
## 16    23 control    Time1     70     1
## 17     6 thyroxin   Time0     52     0
## 18    24 control    Time1     71     1
## 19    10 thiouracil Time2    100     2
## 20    18 control    Time1     86     1
```
...
\includegraphics{20-repeated-measures_files/figure-latex/unnamed-chunk-26-1} 
...
## Observations: 200
## Variables: 6
## $ epi_1    <dbl> 89.572453, -55.502686, ...
## $ epi_2    <dbl> 253.002500, -1.448935, ...
## $ epi_3    <dbl> 213.86716, 26.04160, 26...
## $ epi_4    <dbl> 209.46932, 258.68919, 1...
## $ location <dbl> 1, 1, 1, 1, 1, 1, 1, 1,...
## $ child    <dbl> 1, 2, 3, 4, 5, 6, 7, 8,...
```
...
```r
airport %>% gather(when, epinephrine, c(epi_1, 
    epi_2, epi_3, epi_4))
```
...
```r
airport %>% gather(when, epinephrine, epi_1:epi_4) %>% 
    mutate(time = parse_number(when)) %>% ggplot(aes(x = time, 
    y = epinephrine)) + geom_point()
```


\includegraphics{20-repeated-measures_files/figure-latex/unnamed-chunk-42-1} 
...
airport %>% gather(when, epinephrine, epi_1:epi_4) %>% 
    mutate(time = parse_number(when)) %>% ggplot(aes(x = time, 
    y = epinephrine, colour = factor(child), group = factor(child))) + 
    geom_point() + geom_line()
```


\includegraphics{20-repeated-measures_files/figure-latex/unnamed-chunk-43-1} 
...
```r
airport %>% gather(when, epinephrine, epi_1:epi_4) %>% 
    ggplot(aes(x = when, y = epinephrine, colour = factor(child), 
        group = factor(child))) + geom_point() + 
    geom_line()
```


\includegraphics{20-repeated-measures_files/figure-latex/unnamed-chunk-44-1} 
...
```r
airport %>% gather(when, epinephrine, epi_1:epi_4) %>% 
    mutate(time = parse_number(when)) %>% mutate(fchild = factor(child)) %>% 
    ggplot(aes(x = time, y = epinephrine, colour = fchild, 
        group = fchild)) + geom_point() + geom_line()
```


\includegraphics{20-repeated-measures_files/figure-latex/unnamed-chunk-45-1} 
...
\includegraphics{20-repeated-measures_files/figure-latex/unnamed-chunk-46-1} 
...
airport %>% gather(when, epinephrine, epi_1:epi_4) %>% 
    ...
    ggplot(aes(x = time, y = epinephrine, colour = fchild, 
        group = fchild)) + geom_point() + geom_line() + 
    guides(colour = F) + facet_wrap(~location)
```


\includegraphics{20-repeated-measures_files/figure-latex/unnamed-chunk-47-1} 
...
\includegraphics{20-repeated-measures_files/figure-latex/unnamed-chunk-56-1} 
...
\includegraphics{20-repeated-measures_files/figure-latex/unnamed-chunk-57-1} 
...
\includegraphics{20-repeated-measures_files/figure-latex/unnamed-chunk-58-1} 
...
\includegraphics{20-repeated-measures_files/figure-latex/unnamed-chunk-62-1} 
