...
handed the shirt directly  by the sales associate, without the ``other
customer'' being involved at all. There were thus five treatments:
...
(a)[2] Read in and display the data. How many observations do you
...
(b)[2] Turn `treatment` into a `factor` in your data
...
(c)[2] List the treatments in the order that they are in within
...
(e)[3] The experimenters wanted to compare four specific things in
their analysis:
...
(f)[2] Pick two of your contrasts (doesn't matter which two) and
...
(g)[2] Collect all your contrasts together into a matrix and
...
(h)[2] Predict evaluation score from treatment \emph{as a
regression}, and display the results.
...
(i)[4] For each of your contrasts, assess whether or not it is
