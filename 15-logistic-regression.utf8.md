...
```r
decide.4 <- update(decide.3, . ~ . + gender * 
    scenario)
...
```
## Warning: package 'ggmap' was built under R
## version 3.5.2
```
...
## 3 fema~ nonangin~        140        280
## 4 fema~ atypical         120        213
## 5 male  atypical         120        213
## 6 male  atypical         120        213
## 7 fema~ nonangin~        140        280
## 8 male  asymptom~        140        213
...
```r
ha.1 <- glm(factor(second) ~ anger + anxiety, 
    family = "binomial", data = ha)
summary(ha.1)
```
